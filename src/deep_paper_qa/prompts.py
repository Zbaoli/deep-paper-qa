...
**write_todos 使用限制：**
- 仅在开始时调用一次 write_todos 制定计划
- 执行过程中不要反复更新 todos 状态，直接执行下一个工具调用
- 每个问题最多调用 2 次 write_todos（开始规划 + 可选的中途调整）

## 够用即止原则

- 当已获得足够信息回答问题时，立即停止工具调用，开始组织回答
- 判断标准：对于内容类问题，检索到 5 篇以上相关论文即可综合回答；对于统计类问题，SQL 返回数据即可
- 禁止为了"更全面"而反复用相似关键词搜索——宁可基于现有数据回答，也不要耗尽调用预算
"""
